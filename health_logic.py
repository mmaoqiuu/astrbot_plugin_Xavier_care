...
# ---------------------------------------------------------------------------
# 经期推算：从历史 in_period 记录里算「当前第几天 / 距下次几天 / 下次预测日期」。
# 不依赖任何外部数据，纯从已存文件推算。
# ---------------------------------------------------------------------------


def compute_period_status(data_dir: Path, default_cycle: int = 28) -> dict:
    """扫描所有日期文件，推算当前经期状态。

    返回：
      {
        "in_period": bool,          # 今天是否在经期
        "period_day": int|None,     # 今天是经期第几天（不在经期则 None）
        "last_start": str|None,     # 最近一次经期起始日（YYYY-MM-DD）
        "next_start_date": str|None,# 下次经期预测日期（YYYY-MM-DD）
    ...
    period_days = []
    for d in dates:
        rec = load_by_date(data_dir, d)
        if rec and rec.get("in_period") is True:
    ...
    for d in period_days[1:]:
        prev = datetime.strptime(cur[-1], "%Y-%m-%d").date()
    ...
    last_end = last_seg[-1]
    last_len = (datetime.strptime(last_end, "%Y-%m-%d").date()
                - datetime.strptime(last_start, "%Y-%m-%d").date()).days + 1

    # 平均周期：用最近几次经期的起始日间隔
    if len(segments) >= 2:
        gaps = []
        for i in range(1, len(segments)):
            a = datetime.strptime(segments[i - 1][0], "%Y-%m-%d").date()
            b = datetime.strptime(segments[i][0], "%Y-%m-%d").date()
            gaps.append((b - a).days)
        # 去掉极端值（<15 或 >60 天，多半是数据断档）
        gaps = [g for g in gaps if 15 <= g <= 60]
        avg_cycle = round(sum(gaps) / len(gaps)) if gaps else default_cycle
    else:
    ...
    lengths = []
    for seg in segments:
        a = datetime.strptime(seg[0], "%Y-%m-%d").date()
        b = datetime.strptime(seg[-1], "%Y-%m-%d").date()
        lengths.append((b - a).days + 1)
    avg_length = round(sum(lengths) / len(lengths)) if lengths else last_len

    # 今天是否在经期、第几天
    today = dates[-1]
    in_period_today = today in last_seg
    period_day = None
    if in_period_today:
        period_day = (datetime.strptime(today, "%Y-%m-%d").date()
                      - datetime.strptime(last_start, "%Y-%m-%d").date()).days + 1

    # 下次预测日期 + 距下次天数。
    # 无论今天在不在经期，都给出「这次起始 + 一个周期」的预测。
    today_d = datetime.strptime(today, "%Y-%m-%d").date()
    next_start_d = (datetime.strptime(last_start, "%Y-%m-%d").date()
                    + timedelta(days=avg_cycle))
    days_to_next = (next_start_d - today_d).days

    return {
        "in_period": in_period_today,
        "period_day": period_day,
        "last_start": last_start,
        "next_start_date": next_start_d.isoformat(),
...
def format_report(data: dict | None) -> str:
    ...
    # 经期：优先读「单条数据里的 in_period」，同时用推算补上「第几天」
    in_period = data.get("in_period")
    ps = data.get("_period_status") or {}
    if isinstance(in_period, bool):
        if in_period:
            phrase = _flow_phrase(data.get("period_flow"))
            day_n = ps.get("period_day")
            if day_n:
                lines.append(
                    f"经期：在经期，第 {day_n} 天"
                    + (f"，经量{phrase}。" if phrase else "。")
                )
            else:
                lines.append(
    ...
    # 下次经期预测：无论今在不在经期，都补一行（来自推算）。
    next_date = ps.get("next_start_date")
    if next_date:
        try:
            d = datetime.strptime(next_date, "%Y-%m-%d")
            lines.append(f"下次经期预计 {d.month} 月 {d.day} 日开始。")
        except Exception:
            pass
